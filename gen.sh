...
for i in *.$DIR; do
	SIZE=`du -h "$i" | cut -f 1`
	echo "<p><a href='$i'>$i</a> ($SIZE)</p>" >> $FILE
	if [ "$DIR" == "mp3" ]; then
		LEN=`afinfo -r "$i" | grep duration | cut -d ' ' -f 3 | cut -d '.' -f 1`
		ENCODED="$(perl -MURI::Escape -e 'print uri_escape($ARGV[0]);' "$i")"
		echo "file: $i"
		echo "encoded: $ENCODED"
		echo "<object type=\"application/x-shockwave-flash\" data=\"player.swf\" height=\"30\" width=\"200\"><param name=\"wmode\" 
		...
		value=\"way=${ENCODED}&amp;swf=http://flv-mp3.com/i/pic/ump3player_500x70.swf&amp;w=300&amp;h=45&amp;time_seconds=${LEN}&amp;autoplay=0&amp;q=&amp;skin=grey&amp;volume=10amp;comment=\" />
		</object>" >> $FILE
	fi;
done
